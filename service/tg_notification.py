...
def request_tg_message(full_message):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if result.get('ok'):
                service.logger.logger_service.info("Уведомление в телеграм успешно отправлено")
            ...
            if attempt < max_attempts - 1:
                ...
                delay *= 2
                continue
            service.logger.logger_service.error("Не удалось сделать запрос к API Telegram", exc_info=True)
...
def send_tg_message(message):
    ...
    try:
        ...
        service.logger.logger_service.error("Не удалось сформировать сообщение для отправки уведомления", exc_info=True)