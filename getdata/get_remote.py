...
def get_anydesk_id():
    ...
    try:
        ...
        service.logger.logger_service.error(f'Error: ошибка при получении anydesk_id из {conf_path}', exc_info=True)
...
def get_user_appdata(target_folder_path):
    service.logger.logger_service.debug(f"Пытаемся найти домашннюю директорию активного пользователя")
    try:
            ...
            if session['State'] == win32ts.WTSActive:
                ...
                if user_token:
                    ...
                    if os.path.exists(user_path):
                        service.logger.logger_service.debug(f"Найден активный пользователь, его домашняя директория: '{user_path}'")
                        return user_path
    ...
    service.logger.logger_service.debug(f"Пытаемся найти пользователя, домашнняя директория которого содержит '{target_folder_path}'")
    try:
        ...
        service.logger.logger_service.debug(f"Список пользователей, домашняя директория которых содержит искомый путь '{target_folder_path}':\n{users_with_folder}")
        ...
        if users_with_folder:
            ...
            service.logger.logger_service.debug(f"Будет использоваться домашняя директория пользователя: '{latest_user}'")
            return home_path
...
#     except Exception:
#         service.logger.logger_service.error(f"Error: Не удалось получить информацию о диске", exc_info=True)
